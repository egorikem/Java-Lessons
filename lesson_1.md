...
В [Пример 15]() если смысловой недочет: а что будет, если `number` равен 3? Первое условие не выполнется, так как 3 == 1 – это ложь, и второе условие тоже не выполнится, так как 2 == 1. Проблема в том, что программа тогда не напишет ничего! А мы хотим, чтобы если это не 1 или 2, тогда выводить на экран "Неизвестное число". 
